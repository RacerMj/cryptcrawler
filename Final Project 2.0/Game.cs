...
namespace Final_Project_2._0
{
    public class Game
    {
        ...
        public Game(DungeonForm f)
        {
            ...
            //Get current room
            //Describe room
            gameForm.showText(whatRoomPlayerIsIn.getDescription());
        ...
        private void doCommand()
        {
            ...
            gameForm.showTextLine("*" + box1Text + "*" + System.Environment.NewLine);
            ...
            if (player.currentHitpoints < 0)
            {
                ...
                {
                    gameForm.showTextLine("You are dead. You can do nothing. Leave the game.");
                }
            }
            else
            {
                ...
                else if (split[0] == "MOVE")
                {
                    ...
                    if (split.Length > 1)//&& whatRoomPlayerIsIn != 0 && split[1] == "BACK")
                    {
                        ...
                        {
                            gameForm.showTextLine("You cannot go that way.");
                    ...
                    {
                        gameForm.showTextLine("Where would you like to go?");
                    }
                }
                else if (split[0] == "ATTACK")
                {
                    ...
                    if (split.Count() == 1)
                    {
                        gameForm.showText("What would you like to attack?");
                    }
                    ...
                    else if (monsters != null)
                    {
                        ...
                        {
                            gameForm.showText("There is no such creature. Specify the monster, e.g. RAT1");
                            return;
                        }
                        ...
                        if (targetCreature.currentHitpoints >= 0)
                        {
                            ...
                            if (random.Next(1, 20) < targetCreature.armourClass)
                            {
                                ...
                                gameForm.showTextLine(playerMissDescription[random.Next(0, 4)]);
                                monsterAttack();
                            }
                            else
                            {
                                if (player.weapon != null)
                                {
                                    ...
                                    gameForm.showTextLine(playerHitDescription[random.Next(0, 4)] + " You deal " + player.weapon.weaponDamage + " points of damage to the foul beast.");
                                }
                                else
                                {
                                    gameForm.showTextLine(" You deal " + player.creatureDamage + " points of damage to the foul beast.");
                                }
                                if (targetCreature.currentHitpoints < 1)
                                {
                                    gameForm.showText(System.Environment.NewLine + " The " + targetCreature.creatureName + monsterDeathDescription[random.Next(0, 4)]);
                    ...
                    {
                        gameForm.showText("There are no monsters to attack.");
                    }
                ...
                else if (split[0] == "EQUIP")
                {
                    ...
                    if (weapons != null)
                    {
                        if (split.Length > 1)
                        {
                            for (int i = 0; i < weapons.Count; i++)
                            {
                                if (split[1] == weapons[i].weaponName.ToUpper())
                                {
                                    ...
                                    gameForm.showText("You have equipped a " + player.weapon.weaponName + ".");
                                    break;
                                }
                                else
                                {
                                    gameForm.showText("Uh uh");
                                }
                        ...
                        {
                            gameForm.showText(System.Environment.NewLine + "What would you like to equip?");
                        }
                    }
                    else
                    {
                        gameForm.showText(System.Environment.NewLine + "There are no weapons to equip here.");
                    }
                ...
                else if (split[0] == "LOOT")
                {
                    ...
                    if (split.Length > 1)
                    {
                        ...
                        if (targetCreature == null)
                        {
                            gameForm.showText("There is no such things to loot. Specify the thing, e.g. RAT1");
                        }

                        else if (targetCreature.isLooted)
                        {
                            gameForm.showText("This thing has already been looted.");
                        }
                        else
                        {
                            ...
                            if (treasureChance <= targetCreature.chanceOfTreasure)
                            {
                                ...
                                gameForm.showTextLine("You found a " + t.description + " worth " + t.value + " gold pieces.");
                            }
                            else
                            {
                                gameForm.showTextLine("You found nothing");
                    ...
                    {
                        gameForm.showText("What would you like to loot?");
                    }
                }
                else if (split[0] == "INVENTORY")
                {
                    if (player.creatureTreasures != null)
                    {
                        ...
                        {
                            gameForm.showTextLine("You have nothing");
                        }
                        else
                        {
                            gameForm.showTextLine("You have:");
                            for (int i = 0; i < player.creatureTreasures.Count; i++)
                            {
                                gameForm.showTextLine("A" + player.creatureTreasures[i].description);
                            }
                        }
                    }
                    else
                ...
                {
                    gameForm.showTextLine("You have the following commands:" + System.Environment.NewLine);
                    gameForm.showTextLine("LOOK - Describes your environment" + System.Environment.NewLine);
                    gameForm.showTextLine("MOVE <DIRECTION> - Moves your character to a new room" + System.Environment.NewLine);
                    gameForm.showTextLine("ATTACK <CREATUREnum> - Attacks a creature" + System.Environment.NewLine);
                    gameForm.showTextLine("EQUIP <WEAPONNAME> - Equips a weapon from the floor" + System.Environment.NewLine);
                    gameForm.showTextLine("LOOT <THING> - Loots a decoration for a treasure" + System.Environment.NewLine);
                    gameForm.showTextLine("INVENTORY - Displays your treasures" + System.Environment.NewLine);
                    gameForm.showTextLine("STATUS - Displays your hp and weapon" + System.Environment.NewLine);
                    gameForm.showTextLine("QUIT - Immediately quits the game" + System.Environment.NewLine);
                    gameForm.showTextLine("HELP - Lists commands" + System.Environment.NewLine);
                }
                else if (split[0] == "STATUS")
                {
                    gameForm.showText(System.Environment.NewLine + "You have " + player.currentHitpoints + "/" + player.maxHitpoints + " hitpoints.");
                    gameForm.showText(System.Environment.NewLine + "You have a " + player.weapon.getNameAndDescription() + System.Environment.NewLine + " that does 1-" + player.weapon.weaponDamage + " points of damage.");
                }
                ...
                {
                    gameForm.showText("I do not understand that command.");
                }
            }
